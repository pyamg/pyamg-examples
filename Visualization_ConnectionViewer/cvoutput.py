...
def cvoutput2d(filename, V, A):
    ...
    """
    cvoutput2dv(filename, np.asarray(V[:, 0]), np.asarray(V[:, 1]), A)
...
def cvoutput2dp(filename, Vx, Vy, p1, p2, A):
    ...
    for i, j, v in zip(A.row, A.col, A.data):
        f.write(repr(int(p1[i])) + " " +
                repr(int(p2[j])) + " " + repr(v) + "\n")
...
def outputML(filename, V, mls):
    ...
    for L in range(len(mls.levels) - 1):
        R = mls.levels[L].R.tocoo()
        parent1 = array('I', range(R.shape[1]))
        parent2 = array('I', range(R.shape[0]))
        bCoarse = [0 for i in range(R.shape[1])]
        for i, j, v in zip(R.row, R.col, R.data):
            if v == 1.0:
                parent2[i] = j
                bCoarse[j] = 1
        bFine = [1 - bCoarse[i] for i in range(R.shape[1])]
        ...
        cvoutput2dp(filename + "_A" + repr(L + 1) + ".mat", Vx,
                    Vy, parent1, parent1, mls.levels[L + 1].A)
    # i = len(mls.levels)-1
