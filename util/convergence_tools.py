...
def print_cycle_history(resvec, ml, verbose=False, plotting=False):
    ...
    print('               work: %6.3f' %
          (-ml.cycle_complexity() / scipy.log10(avg_convergence_factor)))
    print('')
    ...
    if(verbose):
        ...
        for i in range(0, len(resvec)):
            if(i > 0):
                ...
                ifactor = ifactors[-1]
                # running arith mean
                aafactor = numpy.mean(ifactors)
                gafactor = (iresvec[-1] / iresvec[0]
                            )**(1.0 / (i + 1))    # geometric mean

                ocx = ml.cycle_complexity()
                # current work-per-digit
                iwork = - ocx / scipy.log10(gafactor)
...
def print_scalability(
    ...
    for n in nlist:
        ...
        table.append([str(int(n)), str(int(nnz[run])), ('%1.3f' %
                                                        factors[run]), ('%1.3f' %
                                                                        complexity[run]), ('%1.3f' %
                                                                                           work[run])])
        run += 1
